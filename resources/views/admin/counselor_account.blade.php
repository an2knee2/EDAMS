    ...
    <div class="mx-auto max-w-screen-xl">
        <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow-sm">
                                ...
                                <form class="p-4 md:p-5" method="POST" action="{{ route('admin.counselor_account.store') }}">
                                    @csrf
                                    <div>
            ...
            <div class="overflow-x-auto">
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody id="counselor-table-body">
                        ...
                        <tr class="border-b">
                            ...
                            <th class="px-4 py-3 font-medium text-gray-700 whitespace-nowrap">{{ $counselor->id_number }}</th>
                            <td class="px-4 py-3 text-gray-700 whitespace-nowrap">
                                {{ $counselor->first_name }} 
                                @if($counselor->middle_name) {{ strtoupper(substr($counselor->middle_name, 0, 1)) }}.@endif 
                                {{ $counselor->last_name }} 
                                @if($counselor->ext_name) , {{ $counselor->ext_name }}@endif
                            </td>
                            <td class="px-4 py-3 text-gray-700 whitespace-nowrap">{{ $counselor->sex }}</td>
                            <td class="px-4 py-3 text-gray-700 whitespace-nowrap">BiPSU Guidance Counselor</td>
                            <td class="px-4 py-3 text-gray-700 whitespace-nowrap">{{ $counselor->email }}</td>
                            <td class="px-4 py-3 text-gray-700 whitespace-nowrap status-column-{{ $counselor->id }}">
                                <div class="flex items-center">
                                    <span class="h-3 w-3 rounded-full mr-2 
                                        {{ $counselor->status == 'Activated' ? 'bg-green-500' : 'bg-red-500' }}">
                                    </span>
                                    <span class="status-text">{{ $counselor->status }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                <button id="dropdown-button-{{ $counselor->id }}" data-dropdown-toggle="dropdown-{{ $counselor->id }}" class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none" type="button">
                ...
                <div id="no-counselors-message" class="text-center font-serif text-gray-600 mt-6 hidden">
                ...
                @else(isset($counselors) && count($counselors) == 0)
                    <div class="text-center font-serif text-gray-600 mt-6">
                        ...
                        No counselor accounts added yet.
                    </div>
                @endif
...
<script>
    function updateStatus(counselorId, status) {
        ...
        .then(data => {
            if (data.success) {
                document.querySelector('.status-column-' + counselorId + ' .status-text').innerText = data.new_status;
                let indicator = document.querySelector('.status-column-' + counselorId + ' span');
    ...
    function filterTable(searchTerm) {
        ...
        document.querySelectorAll('#counselor-table-body tr').forEach(row => {
            const counselorName = row.querySelector('td:nth-child(3)').innerText.toLowerCase();
            const status = row.querySelector('.status-text').innerText;

            const matchesSearch = counselorName.includes(searchTerm);
        ...
        document.getElementById('no-counselors-message').style.display = hasVisibleRows ? 'none' : 'block';
    }
    ...
    function paginateTable() {
        const rows = document.querySelectorAll('#counselor-table-body tr');
